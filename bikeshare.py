...
def get_filters():
    ...
    month = input(
        "Which month would you like to know about, we have the first half from january until june? type all for the whole period\n>>"
    ).lower()
    ...
    day = input(
        "Which day would you like to know about?, (all, monday, tuesday, ETC)\n>>"
    ).lower()
    while day not in days:
        print(
            "just make sure that you either type a day from saturday till friday, or all for all days\n>>"
        )
        day = input(
            "Which day would you like to know about?, (all, monday, tuesday, ETC)\n>>"
        ).lower()
...
def time_stats(df):
    ...
    print(
        "the most common month for the selected period is: {} Note: 1 for january, 2 for february, ETC:\n>>".format(
            common_month
        )
    )
...
def trip_duration_stats(df):
    ...
    print(
        "the total travel time for the selcted period: {}\n>>>".format(
            total_travel_time
        )
    )
    ...
    print(
        "the average travel time for the selcted period: {}\n>>>".format(
            avg_travel_time
        )
    )
...
def user_stats(df):
    ...
    if city != "washington":
        ...
        print(
            "the count of each gender for bikeshare users is: {}\n>>>".format(
                gender_count
            )
        )
...
def raw_data(df):
    while True:
        display = input(
            "would you like to display random samples and quick statistics\n>>"
        ).lower()
        if display == "yes":
